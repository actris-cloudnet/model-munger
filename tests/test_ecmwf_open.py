...
def test_extract_profiles(tmp_path):
    ...
    with netCDF4.Dataset(output_files[0]) as nc:
        ...
        assert_allclose(
            nc["latitude"][:], np.array(sites[0]["latitude"]), rtol=0, atol=0.25 / 2
        )
        assert_allclose(
            nc["longitude"][:], np.array(sites[0]["longitude"]), rtol=0, atol=0.25 / 2
        )
        assert_allclose(nc["pressure"][:], [100_000, 10_000])
        assert_allclose(
            nc["t"][:], [[273.766754, 214.335098], [271.976425, 213.248489]]
        )
        assert_allclose(nc["t2m"][:], [273.366211, 270.937653])
        assert_allclose(
            nc["sot"][:], [[273.004318, 272.756409], [272.97612, 272.759644]]
        )
